...
public class rtwView extends FrameLayout {
	public class BookCellClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			Log.v(TAG, "书籍id为："+layout.getTag().toString());
			int Book_id=Integer.parseInt(layout.getTag().toString());
			Book theBook=books.get(Book_id-1);//数组下标从零开始
	...
	private void initUI(Context context) {
		...
		for (int i = 0; i < imgUrls.length; i++) {
			...
			LinearLayout.LayoutParams layoutParams=new LinearLayout.LayoutParams(135, 180);//宽高
			layoutParams.setMargins(5, 0, 5, 0);//边距
			layout.setLayoutParams(layoutParams);
			layout.setGravity(Gravity.CENTER);//对齐方式
			layout.setOrientation(LinearLayout.VERTICAL);//垂直布局(上下)
			layout.setId(i);
			...
			imageLoader.displayImage(imgUrls[i], imageView,options);//设置图片
